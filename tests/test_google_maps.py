...
def test_get_location(monkeypatch):
    def mock_request_get(self, address):
        return {
            'results': [{
                    ...
                    'geometry': {
                        'bounds': {'northeast': {'lat': 48.8709496, 'lng': 2.3174997}, 'southwest': {'lat': 48.8698034, 'lng': 2.315718}},
                        'location': {'lat': 48.8701231, 'lng': 2.3165784},
                        'location_type': 'ROOFTOP',
                        'viewport': {'northeast': {'lat': 48.8717254802915, 'lng': 2.317957830291502}, 'southwest': {'lat': 48.8690275197085, 'lng': 2.315259869708498}}
                    },
                    'place_id': 'ChIJi-rDls5v5kcRad3lk5loQiI', 'types': ['premise']
            }], 'status': 'OK'
        }

    monkeypatch.setattr(
        'grand_py_app.apis.google_maps.GoogleMaps.request_get', mock_request_get
    )
    ...
    assert maps.get_location(address) == expected_result
